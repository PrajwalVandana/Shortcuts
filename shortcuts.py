...
class Network:
    ...
    def depict(self, r=200, node_shape=0, node_size=30, node_rot=0, node_positions=None,
        ...
        if node_positions is None:  # adding node positions
            node_positions = {}
...
def months(numbers=True):
    ...
    lst = [
        ...
        '5', 'may', '6', 'june', 'jun', '7', 'july', 'jul', '8', 'august', 'aug', '9',
...
def turtle_gif(func, args=None, kwargs=None, fps=30, fname=None, path=None, temp_fname=None, temp_path=None, optimize=False, duration=100, tr=None):
    ...
    optimize, duration: passed when creating the gif, like so:
        `frames[0].save(..., optimize=optimize, duration=duration, ...)` \\
    tr: same Turtle object used by `func`
    """
    if tr is None:
        tr = turtle.Turtle()
...
def mod_inv(n, div):
    ...
    if math.gcd(n, div) != 1:
        ...
        return pow(n, totient(div)-1, div)
...
def convert_color(col_in, out=tuple):
    ...
    if out == tuple:
        ...
        return '#'+''.join([hex(d)[2:] for d in from_decimal(col, 256)])
    elif out == int:
        return col
    else:
        raise TypeError("Colors must be tuples, strings, or integers, not '%s'."
...
def kmeans(pts, k):
    ...
    while divergence > 0:
        ...
        for p in pts:
            clusters[min(means, key=lambda m: _dist(p, m))].add(p)

        divergence = 0
        means = []
        for mean, ps in clusters.items():
            new_mean = [0]*dim
            for p in ps:
                for i in range(dim):
                    new_mean[i] += p[i]
            new_mean = tuple(map(lambda x: x/len(ps), new_mean))

            divergence += _dist(mean, new_mean)
